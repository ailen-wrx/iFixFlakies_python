...
def fix_brittle(project,sha, cleaner_fullpath, victim_fullpath, combination_path,result_path):
    ...
    md5 = get_md5(cleaner_fullpath, victim_fullpath)
    ...
    try:
        ...
        with open(result_path,'a+') as f:
            ...
            result=[project,sha,cleaner_fullpath,victim_fullpath,md5,'PCV_ERROR']
    ...
    if cleaner_brittle_result == 'passed' and brittle_result !='passed':
...
#       print(insert_statement_list)
        ...
        while len(tmp_insert_node) >= 2:
            ...
            while start < len(tmp_insert_node):
                ...
                try:
                    #mp_insert_node = ast.parse(this_round_insert_code)
                    tmp_origin_victim.insert(0, this_round_insert_list)
                    ast.fix_missing_locations(tmp_tree_victim)
                ...
                if can_be_inserted:
                    tmp_origin_victim.remove(this_round_insert_list)
                can_patch_work=(run_brittle(project,md5, combination_path + '::' + '::'.join(victim_fullpath.split('::')[1:])))
...
#        diff=difflib.ndiff(org_contents,contents)
       # print(''.join(diff))
        ...
        print(diff)
        diff_right=run_brittle(project,md5,processed_patch_file)
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~,',diff_right.strip())

        
    with open(result_path,'a+') as f:
        csv_write = csv.writer(f)
        result=[project,sha,cleaner_fullpath,victim_fullpath,md5,brittle_result,cleaner_brittle_result,can_copy_work,patch_time_1st,patch_time_all]
...
def get_md5(cleaner_fullpath, victim_fullpath):
    md5=hashlib.md5()
...
def run_brittle(project,md5, brittle):


#    run_cmd_pv="python3 -m pytest " +brittle+' --csv'+CACHE_DIR+'/'+project+'/{}.csv'.format(md5)
#    print(run_cmd_pv)
 #   os.system(run_cmd_pv)
    
    arg=[brittle,'--csv',CACHE_DIR+project+'/{}.csv'.format(md5)]

    mainargs = ["python3", "-m", "pytest"] + arg
    ...
    brittle_result=status[len(status)-1]

    print(brittle_result)    
    return brittle_result

def run_cleaner_brittle(project,md5, state_setter,brittle):
#    run_cmd_pv="python3 -m pytest " +statesetter+' '+brittle+' --csv'+CACHE_DIR+'/'+project+'/{}.csv'.format(md5)

#    print(run_cmd_pv)
#    os.system(run_cmd_pv) 

    arg=[state_setter,brittle,'--csv',CACHE_DIR+project+'/{}.csv'.format(md5)]
    #capture = io.StdCapture()
    # pytest.main(arg)
    mainargs = ["python3", "-m", "pytest"] + arg
    process = Popen(mainargs)#, stdout=PIPE, stderr=PIPE)
    ...
    csv_result=pd.read_csv(CACHE_DIR +project+ '/{}.csv'.format(md5))
    status=csv_result['status']
    cleaner_brittle_result=status[len(status)-1]

    print(cleaner_brittle_result)
    return cleaner_brittle_result
...
def run_tests_pcv(project, md5, polluter_fullpath, cleaner_fullpath, victim_fullpath):
    ...
    mainargs = ["python3", "-m", "pytest"] + pcv_arg
    process = Popen(mainargs, stdout=PIPE, stderr=PIPE)
    std, err = process.communicate()
...
if __name__ == "__main__":
    ...
    project,sha, cleaner_fullpath, victim_fullpath, combination_path, result_path = sys.argv[1:8]
    fix_brittle(project,sha, cleaner_fullpath, victim_fullpath, combination_path,result_path)
