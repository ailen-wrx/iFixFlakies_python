...
with open("master/src/random_brittle.csv", 'w') as output:
...
for data in dataset: 
    ...
    with open("master/src/random_brittle.csv", 'a') as output:
...
with open("summary_random_brittle.txt", 'w') as summary_out:
    summary_out.write("{}/{} suspected brittles in {}/{} proj. have no state-setters\n".format(
        valid, len(dataset), len(ProjectsLib), len(set(projs))))
    summary_out.write("\t{} susp. brittles have 1+ failing and 1+ passing order in 100 random orders\n".format(nod+od))
    summary_out.write("\t\t{} susp. brittles can reliably fail and pass\n".format(od))
    summary_out.write("\t\t{} susp. brittles cannot reliably fail/pass\n".format(nod))
    summary_out.write("\t{} susp. brittles have no failing or no passing order in 100 random orders\n".format(deter))
